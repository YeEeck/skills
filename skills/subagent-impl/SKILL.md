---
name: subagent-impl
description: 按 to-spec / to-tickets 产出的票据集，为每张票派发一个子代理（点名调用 implement 技能实施）、严格按票面声明的顺序串行实现，上一张票验收全部通过后才开始下一张。当用户指向一组票据（本地票据文件或 tracker issues）并要求派发子代理把它们全部做完时使用。
---
...
逐票汇报：落地了什么、验证结果、是否发生过重派、是否有遗留的问题。
